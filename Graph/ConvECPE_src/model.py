...
from ConvECPE_src.layer import EnhancedLSTM , Biaffine, RoEmbedding, SupConLoss, MultiHeadAttention, NewFusionGate, ERGAT
from ConvECPE_src.method import (
...
class TextClassification(nn.Module):
    def __init__(self, cfg, tokenizer):
        ...
        num_classes = len(cfg['label_dict']) if cfg['emo_cat'] == 'yes' else 2
    ...
    def get_emotion(self, logits, utterance_nums, emotion_labels, emo=True):
        ...
        if self.cfg.emo_cat == 'yes' and emo:
            weights = [1.5] * self.num_classes
            weights[self.neutral_idx] = 1.0
            criterion = nn.CrossEntropyLoss(weight=torch.tensor(weights).to(logits.device))
        else:
