<!--
============================================================================
本目录为「外部框架源码 vendor 副本」，非本仓库原创代码。
- 来源仓库：https://github.com/jjyaoao/HelloAgents
- 分支/版本：learn_version · v0.2.9（与 Datawhale《Hello-Agents》教程正文对应）
- 配套教程：https://github.com/datawhalechina/hello-agents （第七章「构建你的智能体框架」）
- 许可证：CC BY-NC-SA 4.0（署名-非商业性使用-相同方式共享），原 LICENSE 已随附。
  仅用于个人学习/研究，商用前请阅读许可证条款。
- vendor 说明：未走 `pip install`，源码直接放入本目录以便阅读与改写学习。
  已省略上游的 `uv.lock`（体积大、非必需）；其余包结构保持原样。
- 安装依赖：`pip install -r requirements.txt`；如需「完整 import 包」还需
  `pip install huggingface_hub`（上游 builtin 工具会 eager import 评估模块）。
  仅用第七章核心（LLM / Message / Config / Agent / 4 种 Agent / 计算器·搜索工具）
  时，安装 requirements.txt 即可。可 `pip install -e .` 做 editable 安装。
============================================================================
-->
