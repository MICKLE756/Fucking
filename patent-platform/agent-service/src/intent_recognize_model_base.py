...
{"一级意图": "search/analysis/operation/feedback/chitchat", "二级意图": ["具体意图"]}"""
    ...
    def __call__(self, text: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.warning("[远程模型] 超时 (>%ss), 降级", self.timeout)
            return None
        except httpx.ConnectError:
            logger.warning("[远程模型] 连接失败, 降级")
            return None
        except httpx.HTTPStatusError as e:
            logger.warning("[远程模型] HTTP %s, 降级", e.response.status_code)
            return None
        except Exception as e:
            logger.warning("[远程模型] 异常: %s, 降级", e)
            return None
    ...
    def _parse_response(self, data: dict) -> Optional[dict]:
        try:
            ...
            if level1 not in config.INTENT_INFO:
                logger.warning("[远程模型] 非法一级意图: %s, 降级", level1)
                return None
            ...
            logger.info("[远程模型] 意图: %s", intent)
            return intent
        except (KeyError, IndexError, json.JSONDecodeError) as e:
            logger.warning("[远程模型] 响应解析失败: %s, 降级", e)
            return None
